...
def de(fun_opt, methods, bounds, mut=0.8, crossp=0.7, popsize=20, itrs=10):
    ...
    for i in range(itrs):
        for j in range(popsize):
            ...
            if methods == 0:
                if f < fitness[j]:  ####### MRE, RSE
                    fitness[j] = f
                    pop[j] = trial
                    if f < fitness[best_idx]:  ####### MRE, RSE
                        best_idx = j
                        best = trial_denorm
            if methods == 1:
                if f > fitness[j]:  ####### SA
...
def flash(train_input, train_actual_effort, test_input, test_actual_effort, methods, pop_size):
    ...
    while len(List_X) < 201 and life > 0:  # eval_number
        ...
        if methods == 0:
            if candi_pred_value < np.median(List_Y):  ######### for MRE, RSE flash
