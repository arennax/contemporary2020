...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        assert ("Pass start index and step!!")
    start = int(sys.argv[1])
    step = int(sys.argv[2])
    repo_names = get_repo_names("./data/data2.json")[start:start + step]
    existing_results = get_existing_results("./results/")
    token_idx = int(start / step)
    token = list(_token.values())[token_idx]

    for repo_name in sorted(repo_names):
        if repo_name.split("/")[-1] in existing_results:
            print(f"{repo_name} exists, skipping...")
            continue
        miner = Miner(token, debug=False)
        while miner.g.rate_limiting[0] < QUOTA_LIMIT:
